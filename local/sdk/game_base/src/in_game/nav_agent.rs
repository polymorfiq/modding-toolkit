...
pub trait VirtualNavAgent<T> {
    ...
    fn get_world_location(&self) -> FVector {
        ...
        unsafe {
            *(*self.nav_virtual_funcs().get_nav_agent_location())(self.nav_agent().0, Box::into_raw(result_buf))
        }
    }
}

impl VirtualNavAgent<APawn> for APawn {
    fn nav_agent(&self) -> NavAgent<APawn> { NavAgent::new(std::ptr::addr_of!(self.base_nav_agent)) }
}